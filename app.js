...
function renderNetwork() {
  ...
  positions.slice(0, -1).forEach((layer, layerIndex) => layer.forEach((from, index) => positions[layerIndex + 1].forEach((to, targetIndex) => {
    const activation = layers[layerIndex][index] * layers[layerIndex + 1][targetIndex]; const weight = edgeWeight(index, targetIndex, layerIndex); const strength = Math.min(1, activation * (.45 + Math.abs(weight) * .55));
    netCtx.beginPath(); netCtx.moveTo(from.x, from.y); netCtx.lineTo(to.x, to.y); netCtx.lineWidth = .35 + strength * 1.25; netCtx.strokeStyle = weight >= 0 ? `rgba(101,217,255,${.05 + strength * .75})` : `rgba(243,120,189,${.04 + strength * .45})`; netCtx.stroke();
  })));
  positions.forEach((layer, layerIndex) => layer.forEach((node, index) => { const active = layers[layerIndex][index]; netCtx.beginPath(); netCtx.arc(node.x, node.y, layerIndex === 3 ? 4 : 3, 0, Math.PI * 2); netCtx.fillStyle = inputMode === 'nonsense' && active > .35 ? '#f378bd' : `rgba(101,217,255,${.35 + active * .65})`; netCtx.shadowColor = netCtx.fillStyle; netCtx.shadowBlur = active > .35 ? 14 : 0; netCtx.fill(); netCtx.shadowBlur = 0; }));
}
